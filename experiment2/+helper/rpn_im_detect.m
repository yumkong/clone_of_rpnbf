function [pred_boxes, scores] = rpn_im_detect(conf, caffe_net, im, im_idx)
...
% --------------------------------------------------------    
    ...
    anchors = helper.rpn_get_anchor_position(conf, size(im), conf.test_scales, featuremap_size);
    pred_boxes = helper.rpn_bbox_transform_inv(anchors, box_deltas);
      % scale back
