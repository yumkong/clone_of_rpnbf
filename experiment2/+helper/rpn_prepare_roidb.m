function [image_roidb, bbox_means, bbox_stds] = rpn_prepare_roidb(conf, imdbs, roidbs, bbox_means, bbox_stds)
...
function [image_roidb, means, stds] = append_bbox_regression_targets(conf, image_roidb, means, stds)
    ...
    for i = 1:num_images
       ...
       [anchors, im_scales] = helper.rpn_get_anchor_position(conf, image_roidb_cell{i}.im_size);
...
%   negative: [-1, zero(regression_label)]
    ...
    ex_gt_overlaps = helper.boxoverlap(ex_rois, gt_rois);
    ...
    [regression_label] = helper.rpn_bbox_transform(src_rois, target_rois);
