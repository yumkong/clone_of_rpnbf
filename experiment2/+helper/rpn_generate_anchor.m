function anchors = rpn_generate_anchor(cache_path, varargin)
...
%% inputs
    ...
    ip.addParamValue('add_size',        [],             @ismatrix); %1112 a new param: add anchor size that is != 2^x
    ip.addParamValue('ignore_cache',    false,          @islogical);
    ip.parse(cache_path, varargin{:});
    opts = ip.Results;

%%
    if ~opts.ignore_cache
        anchor_cache_file           = fullfile(cache_path, 'anchors');
    end
    try
