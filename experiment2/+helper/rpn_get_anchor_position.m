function [anchors, im_scales] = rpn_get_anchor_position(conf, im_size, target_scale, feature_map_size)
...
function [anchors, im_scale] = proposal_locate_anchors_single_scale(im_size, conf, target_scale, feature_map_size)
    if isempty(feature_map_size)
        im_scale = helper.rpn_get_im_scale(im_size, target_scale, conf.max_size);
