function [im, im_scale] = rpn_prep_im_keepsize(im, im_means, input_scale, min_size, max_size)
%function [im, im_scale] = prep_im_for_blob_keepsize(im, im_means, target_size, max_size)
    ...
    if ~isa(im, 'gpuArray')
        ...
        im_scale = helper.rpn_prep_im_scale_keepsize(size(im), input_scale, min_size, max_size);
        ...
        %0123 changed
        %im_scale = prep_im_for_blob_size_keepsize(size(im), target_size, max_size);
        im_scale = helper.rpn_prep_im_scale_keepsize(size(im), min_size, max_size);
