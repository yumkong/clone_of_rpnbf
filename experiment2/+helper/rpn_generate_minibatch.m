function [input_blobs, random_scale_inds] = rpn_generate_minibatch(conf, image_roidb)
...
% --------------------------------------------------------
    ...
    feat_hei = cell2mat(conf.output_height_map.values({size(im_blob,1)}));
    feat_wid = cell2mat(conf.output_width_map.values({size(im_blob,2)}));
    feat_chl = size(conf.anchors, 1);
    labels_blob = zeros(feat_wid, feat_hei, feat_chl, num_images);
    label_weights_blob = labels_blob;
    bbox_targets_blob = zeros(feat_wid, feat_hei, feat_chl*4, num_images);
    bbox_loss_blob = bbox_targets_blob;
    ...
    for i = 1:num_images
        ...
        %0122 changed
        labels_blob_tmp = reshape(labels, size(conf.anchors, 1), output_size(1), output_size(2));
        label_weights_blob_tmp = reshape(label_weights, size(conf.anchors, 1), output_size(1), output_size(2));
        bbox_targets_blob_tmp = reshape(bbox_targets', size(conf.anchors, 1)*4, output_size(1), output_size(2));
        bbox_loss_blob_tmp = reshape(bbox_loss', size(conf.anchors, 1)*4, output_size(1), output_size(2));
        
        % permute from [channel, height, width], where channel is the
        % fastest dimension to [width, height, channel]
        %0122 changed
        labels_blob(:,:,:,i) = permute(labels_blob_tmp, [3, 2, 1]);
        label_weights_blob(:,:,:,i) = permute(label_weights_blob_tmp, [3, 2, 1]);
        bbox_targets_blob(:,:,:,i) = permute(bbox_targets_blob_tmp, [3, 2, 1]);
...
function [im_blob, im_scales] = get_image_blob(conf, images, random_scale_inds)
    ...
    for i = 1:num_images
        ...
        [im, im_scale] = helper.rpn_get_im(im, conf.image_means, target_size, conf.max_size);
        
        im_scales(i) = im_scale;
        processed_ims{i} = im; 
    end
    
    im_blob = helper.im_list_to_blob(processed_ims);
end
