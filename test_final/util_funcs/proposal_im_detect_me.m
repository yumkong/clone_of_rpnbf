function [pred_boxes_s4_all, scores_s4_all, pred_boxes_s8_all, scores_s8_all, pred_boxes_s16_all, scores_s16_all] = proposal_im_detect_me(conf, caffe_net, im, im_idx)
...
function [blob, im_scales] = get_image_blob(conf, im)
    if length(conf.test_scale_range) == 1
        ...
        [blob, im_scales] = prep_im_for_blob_me(im, conf.image_means, 1, conf.min_test_length, conf.max_test_length);
    else
        % arrayfun: function work on a vector
        [ims, im_scales] = arrayfun(@(x) prep_im_for_blob_me(im, conf.image_means, x, conf.min_test_length, conf.max_test_length), conf.test_scale_range, 'UniformOutput', false);
