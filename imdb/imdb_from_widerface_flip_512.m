function [imdb, roidb] = imdb_from_widerface_flip_512(root_dir, image_set, flip, cache_dir, model_name_base, event_num)
...
try
    ...
    if flip
        ...
        for i = 1:length(imdb.image_ids)
            ...
            % generate flipped image from x0.5, x1 or x2
            if ~exist(flip_image_at(i), 'file')
                imtype = flip_which_image_pool(i);
                if imtype == 1
                    im = im_crop_x05;
                    box_rec = final_bbox_x05;
                elseif imtype == 2
                    im = im_crop_x2;
                    box_rec = final_bbox_x2;
                else
                    im = im_crop_x1;
                    box_rec = final_bbox_x1;
                end
                %0120 added
                if ~isempty(box_rec) 
                    box_ = box_rec;
                ...
                final_bbox_flip = box_rec;
                if 0
                %show new bbox and new image
                bbs_show = final_bbox_flip;
                figure(4), clf;
                imshow(im_crop_flip);
                hold on
                if ~isempty(bbs_show)
                    %0120: different from x01,x1,x2, here [x1,y1,x2,y2]
                    bbs_show(:, 3) = bbs_show(:, 3) - bbs_show(:, 1) + 1;
                    bbs_show(:, 4) = bbs_show(:, 4) - bbs_show(:, 2) + 1;
                    bbApply('draw',bbs_show,'m');
                end
                ...
                %save new bbox and new image
                if 1
                tmpboxdb.image_boxes_flip{i} = final_bbox_flip;
                imwrite(im_crop_flip, flip_image_at(i));  
                end
            end
        end
        img_num = length(imdb.image_ids)*4;
        image_ids = imdb.image_ids;
        %imdb.image_ids(1:4:img_num) = image_ids;
        imdb.image_ids(1:4:img_num) = cellfun(@(x) [x, '_x05'], image_ids, 'UniformOutput', false);
        imdb.image_ids(2:4:img_num) = cellfun(@(x) [x, '_x1'], image_ids, 'UniformOutput', false);
        imdb.image_ids(3:4:img_num) = cellfun(@(x) [x, '_x2'], image_ids, 'UniformOutput', false);
    ...
    imdb.sizes = sizes;
    %save it
    save(cache_imdb, 'imdb','tmpboxdb', 'flip_type_pool', '-v7.3');
end
...
function roidb = roidb_from_wider(imdb, tmpboxdb)
    ...
    for i = 1:length(imdb.image_ids)/4  %2
        ...
        % round is because the widerfaces gt boxes are real numbers
        box_st_flip = box_; %round([box_(:,1) box_(:,2) box_(:,1)+box_(:,3)-1 box_(:,2)+box_(:,4)-1]);

        roidb.rois(i*4-3) = attach_proposals(box_st_x05, imdb.sizes(i*4-3, :), imdb.class_to_id);
        roidb.rois(i*4-2) = attach_proposals(box_st_x1, imdb.sizes(i*4-2, :), imdb.class_to_id);
        roidb.rois(i*4-1) = attach_proposals(box_st_x2, imdb.sizes(i*4-1, :), imdb.class_to_id);
        roidb.rois(i*4) = attach_proposals(box_st_flip, imdb.sizes(i*4, :), imdb.class_to_id);
    end
end
