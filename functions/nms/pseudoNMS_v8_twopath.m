function rects = pseudoNMS_v8_twopath(candi_rects, nms_option, img)
        ...
        if numel(index) == 1
            ...
            for ii = 1:length(new_idx)
                if ~combined_flag(ii)
                    ...
                    s = max(h,0) .* max(w,0);
                    % 1006 changed to make it more strict
                    nearby_idx = s ./ (local_area_all(ii) + local_area_all - s) >= 0.4;%0.5
                ...
                if (cnt>=2) && all(new_rects(~combined_flag, 5)<0.995)
        ...
        if numel(index) == 1
            ...
            for ii = 1:length(new_idx)
                if ~combined_flag(ii)
                    ...
                    s = max(h,0) .* max(w,0);
                    % 1006 changed to make it more strict
                    nearby_idx = s ./ (local_area_all(ii) + local_area_all - s) >= 0.45;%0.1
                    combined_flag(nearby_idx) = true;
                    % use ii to represent all nearby boxes
                    % 0105 changed
