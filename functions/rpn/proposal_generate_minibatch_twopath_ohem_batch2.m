function [input_blobs, random_scale_inds] = proposal_generate_minibatch_twopath_ohem_batch2(conf, image_roidb)
...
%     end
    ...
    rois_per_image = conf.batch_size / num_images;  %160/2 = 80 bboxes/image: (pos 20, neg 60)
    fg_rois_per_image = round(rois_per_image * conf.fg_fraction);  % 80*0.25 = 20
    ...
    %for res23
    feat_hei_res23 = cell2mat(conf.output_height_res23.values({size(im_blob,1)}));
    feat_wid_res23 = cell2mat(conf.output_width_res23.values({size(im_blob,2)}));
    feat_chl_res23 = size(conf.anchors_res23, 1);
    labels_blob_res23 = zeros(feat_wid_res23, feat_hei_res23, feat_chl_res23, num_images);
    label_weights_blob_res23 = labels_blob_res23;
    bbox_targets_blob_res23 = zeros(feat_wid_res23, feat_hei_res23, feat_chl_res23*4, num_images);
    bbox_loss_blob_res23 = bbox_targets_blob_res23;
    %for res45
    feat_hei_res45 = cell2mat(conf.output_height_res45.values({size(im_blob,1)}));
    feat_wid_res45 = cell2mat(conf.output_width_res45.values({size(im_blob,2)}));
    feat_chl_res45 = size(conf.anchors_res45, 1);
    labels_blob_res45 = zeros(feat_wid_res45, feat_hei_res45, feat_chl_res45, num_images);
    label_weights_blob_res45 = labels_blob_res45;
    bbox_targets_blob_res45 = zeros(feat_wid_res45, feat_hei_res45, feat_chl_res45*4, num_images);
    bbox_loss_blob_res45 = bbox_targets_blob_res45;
    
    for i = 1:num_images
        % ###################################
        % note that the only differece between ohem and non-ohem is here!!
        ...
        labels_blob_res23_tmp = reshape(labels_res23, size(conf.anchors_res23, 1), output_size_res23(1), output_size_res23(2));
        label_weights_blob_res23_tmp = reshape(label_weights_res23, size(conf.anchors_res23, 1), output_size_res23(1), output_size_res23(2));
        bbox_targets_blob_res23_tmp = reshape(bbox_targets_res23', size(conf.anchors_res23, 1)*4, output_size_res23(1), output_size_res23(2));
        bbox_loss_blob_res23_tmp = reshape(bbox_loss_res23', size(conf.anchors_res23, 1)*4, output_size_res23(1), output_size_res23(2));
        % permute from [channel, height, width], where channel is the
        % fastest dimension to [width, height, channel]
        labels_blob_res23(:,:,:,i) = permute(labels_blob_res23_tmp, [3, 2, 1]);
        label_weights_blob_res23(:,:,:,i) = permute(label_weights_blob_res23_tmp, [3, 2, 1]);
        bbox_targets_blob_res23(:,:,:,i) = permute(bbox_targets_blob_res23_tmp, [3, 2, 1]);
        bbox_loss_blob_res23(:,:,:,i) = permute(bbox_loss_blob_res23_tmp, [3, 2, 1]);
        
        % =============== for conv5 =================
        output_size_res45 = cell2mat([conf.output_height_res45.values({img_size(1)}), conf.output_width_res45.values({img_size(2)})]);
        labels_blob_res45_tmp = reshape(labels_res45, size(conf.anchors_res45, 1), output_size_res45(1), output_size_res45(2));
        label_weights_blob_res45_tmp = reshape(label_weights_res45, size(conf.anchors_res45, 1), output_size_res45(1), output_size_res45(2));
        bbox_targets_blob_res45_tmp = reshape(bbox_targets_res45', size(conf.anchors_res45, 1)*4, output_size_res45(1), output_size_res45(2));
        bbox_loss_blob_res45_tmp = reshape(bbox_loss_res45', size(conf.anchors_res45, 1)*4, output_size_res45(1), output_size_res45(2));
        % permute from [channel, height, width], where channel is the
        % fastest dimension to [width, height, channel]
        labels_blob_res45(:,:,:,i) = permute(labels_blob_res45_tmp, [3, 2, 1]);
        label_weights_blob_res45(:,:,:,i) = permute(label_weights_blob_res45_tmp, [3, 2, 1]);
        bbox_targets_blob_res45(:,:,:,i) = permute(bbox_targets_blob_res45_tmp, [3, 2, 1]);
        bbox_loss_blob_res45(:,:,:,i) = permute(bbox_loss_blob_res45_tmp, [3, 2, 1]);
    end
