function [aboxes_s4, aboxes_s8, aboxes_s16] = proposal_test_widerface_ablation_MALF_scale3(conf, varargin)
%function [aboxes_s4, aboxes_s8, aboxes_s16] = proposal_test_widerface_ablation_FDDB_scale3(conf, imdb, varargin)
...
%% inputs
    ...
    cache_dir = fullfile(pwd, 'output', conf.exp_name, 'rpn_cachedir', opts.cache_name, 'MALF_test');
	%cache_dir = fullfile(pwd, 'output', opts.cache_name, imdb.name);
    try
        % try to load cache
        %ld = load(fullfile(cache_dir, ['proposal_boxes_' imdb.name opts.suffix]));
        ld = load(fullfile(cache_dir, ['proposal_boxes_MALF' opts.suffix]));
...
%% testing
        ...
        %0409 added: prepare FDDB data
        dataDir = opts.data_dir;
        
        %0409 masked: MALF dataset does not have a list file
        % just read from '0001.jpg' to '5000.jpg' 
        imgDir = fullfile(dataDir, 'Pics');
        num_images = 5000;
...
%         end
        ...
        for i = 1:num_images
            count = count + 1;
            fprintf('%s: MALF test %d/%d ', procid(), count, num_images);
            th = tic;
            %0409 changed
            %im = imread(imdb.image_at(i));
            %imgFile = fullfile(imgDir, [fileList{i}, '.jpg']);
            imgFile = fullfile(imgDir, sprintf('%04d.jpg', i));
            im = imread(imgFile);
        ...
        save(fullfile(cache_dir, ['proposal_boxes_MALF' opts.suffix]), 'aboxes_s4', 'aboxes_s8', 'aboxes_s16', '-v7.3');
