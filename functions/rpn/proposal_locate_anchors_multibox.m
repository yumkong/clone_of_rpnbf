function [anchors_conv4,anchors_conv5, im_scales] = proposal_locate_anchors_multibox(conf, im_size, target_scale, feature_map_size)
...
% generate anchors for each scale
    ...
    if exist('target_scale', 'var')
        [anchors_conv4,anchors_conv5, im_scales] = func(im_size, conf, target_scale, feature_map_size);
    else
        [anchors_conv4,anchors_conv5, im_scales] = arrayfun(@(x) func(im_size, conf, x, feature_map_size), ...
            conf.scales, 'UniformOutput', false);
    end

end

function [anchors_conv4,anchors_conv5, im_scale] = proposal_locate_anchors_single_scale(im_size, conf, target_scale, feature_map_size)
