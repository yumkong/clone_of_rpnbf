function [image_roidb, bbox_mean_s4, bbox_std_s4, bbox_mean_s8, bbox_std_s8, bbox_mean_s16, bbox_std_s16]...
    = proposal_prepare_image_roidb_ablation_final(conf, imdbs, roidbs, bbox_mean_s4, bbox_std_s4, bbox_mean_s8, bbox_std_s8, bbox_mean_s16, bbox_std_s16)
...
% --------------------------------------------------------   
% liu@1209: NOTE that XXX_conv4 is actually conv4, but for labor convenience, do not change  
        ...
        image_roidb = ...
            cellfun(@(x, y) ... // @(imdbs, roidbs)
                arrayfun(@(z) ... //@([1:length(x.image_ids)])
                    ...
                    'boxes', y.rois(z).boxes(y.rois(z).gt, :), 'class', y.rois(z).class(y.rois(z).gt, :), 'image', [], ...,
                    'bbox_targets_s4', [], 'bbox_targets_s8', [],'bbox_targets_s16', []), ...
        ...
        image_roidb = ...
            cellfun(@(x, y) ... // @(imdbs, roidbs)
                arrayfun(@(z) ... //@([1:length(x.image_ids)])
                    ...
                    'boxes', y.rois(z).boxes, 'class', y.rois(z).class, 'image', [],...
                    'bbox_targets_s4', [], 'bbox_targets_s8', [],'bbox_targets_s16', []), ...
                    [1:length(x.image_ids)]', 'UniformOutput', true),...
                    imdbs, roidbs, 'UniformOutput', false);
    end
    ...
    [image_roidb, bbox_mean_s4, bbox_std_s4, bbox_mean_s8, bbox_std_s8, bbox_mean_s16, bbox_std_s16] = append_bbox_regression_targets(conf, ...
                            image_roidb, bbox_mean_s4, bbox_std_s4, bbox_mean_s8, bbox_std_s8, bbox_mean_s16, bbox_std_s16);
end

function [image_roidb, bbox_mean_s4, bbox_std_s4, bbox_mean_s8, bbox_std_s8, bbox_mean_s16, bbox_std_s16] = append_bbox_regression_targets(conf, image_roidb, ...
    ...
    bbox_targets_s4 = cell(num_images, 1);
    bbox_targets_s8 = cell(num_images, 1);
    bbox_targets_s16 = cell(num_images, 1);
    ...
    for i = 1:num_images
       ...
       [anchors_s4,anchors_s8, anchors_s16, im_scales] = proposal_locate_anchors_ablation_final(conf, image_roidb_cell{i}.im_size);
        
       gt_rois = image_roidb_cell{i}.boxes;
       gt_labels = image_roidb_cell{i}.class;
       im_size = image_roidb_cell{i}.im_size;
       bbox_targets_s4{i} = cellfun(@(x, y) ...
           compute_targets(conf, scale_rois(gt_rois, im_size, y), gt_labels,  x, image_roidb_cell{i}, y), ...
           anchors_s4, im_scales, 'UniformOutput', false);
       bbox_targets_s8{i} = cellfun(@(x, y) ...
           compute_targets(conf, scale_rois(gt_rois, im_size, y), gt_labels,  x, image_roidb_cell{i}, y), ...
           anchors_s8, im_scales, 'UniformOutput', false);
       bbox_targets_s16{i} = cellfun(@(x, y) ...
           compute_targets(conf, scale_rois(gt_rois, im_size, y), gt_labels,  x, image_roidb_cell{i}, y), ...
           anchors_s16, im_scales, 'UniformOutput', false);
    end
    clear image_roidb_cell;
    for i = 1:num_images
        image_roidb(i).bbox_targets_s4 = bbox_targets_s4{i};
        image_roidb(i).bbox_targets_s8 = bbox_targets_s8{i};
        image_roidb(i).bbox_targets_s16 = bbox_targets_s16{i};
    end
    clear bbox_targets_s4 bbox_targets_s8 bbox_targets_s16;
    
    if ~(exist('bbox_mean_s4', 'var') && ~isempty(bbox_mean_s4) && exist('bbox_std_s4', 'var') && ~isempty(bbox_std_s4))
        ...
        for i = 1:num_images
           for j = 1:length(conf.scales)
                targets = image_roidb(i).bbox_targets_s4{j};
        ...
        bbox_mean_s4 = bsxfun(@rdivide, sums, class_counts);
        bbox_std_s4 = (bsxfun(@minus, bsxfun(@rdivide, squared_sums, class_counts), bbox_mean_s4.^2)).^0.5;
    end
    ...
    for i = 1:num_images
        for j = 1:length(conf.scales)
            ...
            gt_inds = find(targets(:, 1) > 0);
            if ~isempty(gt_inds)
                image_roidb(i).bbox_targets_s4{j}(gt_inds, 2:end) = ...
                    bsxfun(@minus, image_roidb(i).bbox_targets_s4{j}(gt_inds, 2:end), bbox_mean_s4);
                image_roidb(i).bbox_targets_s4{j}(gt_inds, 2:end) = ...
                    bsxfun(@rdivide, image_roidb(i).bbox_targets_s4{j}(gt_inds, 2:end), bbox_std_s4);
            end
        end
    end
    
    if ~(exist('bbox_mean_s8', 'var') && ~isempty(bbox_mean_s8) && exist('bbox_std_s8', 'var') && ~isempty(bbox_std_s8))
        % Compute values needed for means and stds
        % var(x) = E(x^2) - E(x)^2
        class_counts = zeros(1, 1) + eps;
        sums = zeros(1, 4);
        ...
        for i = 1:num_images
           for j = 1:length(conf.scales)
                targets = image_roidb(i).bbox_targets_s8{j};
                gt_inds = find(targets(:, 1) > 0);
                if ~isempty(gt_inds)
                    class_counts = class_counts + length(gt_inds); 
                    %0805 added check complex number
        ...
        bbox_mean_s8 = bsxfun(@rdivide, sums, class_counts);
        bbox_std_s8 = (bsxfun(@minus, bsxfun(@rdivide, squared_sums, class_counts), bbox_mean_s8.^2)).^0.5;
    end
    ...
    for i = 1:num_images
        for j = 1:length(conf.scales)
            ...
            gt_inds = find(targets(:, 1) > 0);
            if ~isempty(gt_inds)
                image_roidb(i).bbox_targets_s8{j}(gt_inds, 2:end) = ...
                    bsxfun(@minus, image_roidb(i).bbox_targets_s8{j}(gt_inds, 2:end), bbox_mean_s8);
                image_roidb(i).bbox_targets_s8{j}(gt_inds, 2:end) = ...
                    bsxfun(@rdivide, image_roidb(i).bbox_targets_s8{j}(gt_inds, 2:end), bbox_std_s8);
            end
        end
    end
    
    if ~(exist('bbox_mean_s16', 'var') && ~isempty(bbox_mean_s16) && exist('bbox_std_s16', 'var') && ~isempty(bbox_std_s16))
        % Compute values needed for means and stds
        % var(x) = E(x^2) - E(x)^2
        class_counts = zeros(1, 1) + eps;
        sums = zeros(1, 4);
        ...
        bbox_mean_s16 = bsxfun(@rdivide, sums, class_counts);
        bbox_std_s16 = (bsxfun(@minus, bsxfun(@rdivide, squared_sums, class_counts), bbox_mean_s16.^2)).^0.5;
    end
