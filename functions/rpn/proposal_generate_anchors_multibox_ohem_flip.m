function [anchors_conv4, anchors_conv5, anchors_conv6] = proposal_generate_anchors_multibox_ohem_flip(cache_name, varargin)
...
%% inputs
    ...
    ip.addParamValue('add_size',        [900],          @ismatrix); %1112 a new param: add anchor size that is != 2^x
...
%         anchor_cache_file           = fullfile(anchor_cache_dir, 'anchors');
    ...
    try
        ...
        anchors                 = cellfun(@(x) scale_jitter(x, opts.scales, opts.add_size, opts.base_size), num2cell(ratio_anchors, 2), 'UniformOutput', false);
        anchors                 = cat(1, anchors{:});
        % 1112 added
        half_anchor_num = size(anchors, 1) / 2;
        anchors_conv4 = anchors([1:3 half_anchor_num+1:half_anchor_num+3], :);  % [8  16 32]
        anchors_conv5 = anchors([3:8 half_anchor_num+3:half_anchor_num+8], :);  % [32 64 128 256 360 512]
        anchors_conv6 = anchors([7:10 half_anchor_num+7:half_anchor_num+10], :);  % [360 512 720 900]
        if ~opts.ignore_cache
...
function anchors = scale_jitter(anchor, scales, add_size, base_size)
    scales = scales(:);
    % convert additional anchor-size to anchor-ratio and append to the
    % existing 2^x ratios and sort in ascending order
    scales = sort([scales; (add_size / base_size)']);
    w = anchor(3) - anchor(1) + 1;
    h = anchor(4) - anchor(2) + 1;
    x_ctr = anchor(1) + (w - 1) / 2;
    y_ctr = anchor(2) + (h - 1) / 2;
