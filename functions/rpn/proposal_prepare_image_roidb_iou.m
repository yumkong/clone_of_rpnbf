function image_roidb = proposal_prepare_image_roidb_iou(conf, imdbs, roidbs)
...
%     end
    ...
    image_roidb = append_bbox_regression_targets(conf, image_roidb);
end

function image_roidb = append_bbox_regression_targets(conf, image_roidb)
    ...
    clear bbox_targets;
    % deleted calculate mean and std
end
...
%   negative: [-1, zero(regression_label)]
    ...
    ex_gt_overlaps = boxoverlap_yolo(ex_rois, gt_rois);
    ...
    [regression_label] = fast_rcnn_bbox_transform_iou(src_rois, target_rois);
