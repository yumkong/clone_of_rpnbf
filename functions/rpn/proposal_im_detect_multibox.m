function [pred_boxes, scores] = proposal_im_detect_multibox(conf, caffe_net, im, im_idx)
%function [pred_boxes_conv4, scores_conv4, pred_boxes_conv5, scores_conv5] = proposal_im_detect_multibox(conf, caffe_net, im, im_idx)
...
% --------------------------------------------------------    
    ...
    scores_conv4 = output_blobs{3}(:, :, end);
    ...
    assert(conf.test_binary == false);
    % use softmax estimated probabilities
    scores_conv5 = output_blobs{4}(:, :, end);
    ...
    [pred_boxes_conv5, scores_conv5] = filter_boxes(20, pred_boxes_conv5, scores_conv5);
