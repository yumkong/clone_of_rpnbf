function [anchors_conv4, anchors_conv5, anchors_conv6] = proposal_generate_anchors_ablation_total(cache_name, varargin)
...
%         anchor_cache_file           = fullfile(anchor_cache_dir, 'anchors');
    ...
    try
        ...
        anchors_conv4 = anchors([1 half_anchor_num+1], :);  % [8]
        anchors_conv5 = anchors([2:5 half_anchor_num+2:half_anchor_num+5], :);  % [16 32 64 128]
        anchors_conv6 = anchors([5:7 half_anchor_num+5:half_anchor_num+7], :);  % [128 256 480]
        if ~opts.ignore_cache
