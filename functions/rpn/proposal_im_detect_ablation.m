function [pred_boxes, scores] = proposal_im_detect_ablation(conf, caffe_net, im, im_idx)
...
function [blob, im_scales] = get_image_blob(conf, im)
    if length(conf.test_scales) == 1
        ...
        [ims, im_scales] = arrayfun(@(x) prep_im_for_blob_keepsize(im, conf.image_means, x, conf.test_max_size), conf.test_scales, 'UniformOutput', false);
