function [pred_boxes, scores] = proposal_im_detect_conv4plus5(conf, caffe_net, im, im_idx)
...
function [blob, im_scales] = get_image_blob(conf, im)
    if length(conf.test_scales) == 1
        [blob, im_scales] = prep_im_for_blob_conv4plus5(im, conf.image_means, conf.test_scales, conf.test_max_size);
