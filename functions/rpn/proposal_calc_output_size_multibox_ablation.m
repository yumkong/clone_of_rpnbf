function [output_width_map_conv34, output_height_map_conv34, ...
          output_width_map_conv5, output_height_map_conv5, ...
          output_width_map_conv6, output_height_map_conv6] = proposal_calc_output_size_multibox_ablation(conf, test_net_def_file, output_map_save_name)
...
%     caffe.init_log(fullfile(pwd, 'caffe_log'));
    ...
    try
		...
		input = 512;
        output_w_conv34 = nan(size(input));
        output_h_conv34 = nan(size(input));
        ...
        for i = 1:length(input)
            ...
            im_blob = single(zeros(s, 128, 3, 1)); %1210: 500-->504
            net_inputs = {im_blob};
            ...
            cls_score_conv34 = caffe_net.blobs('proposal_cls_score_conv34').get_data();
            % w x h x ch x num
            output_w_conv34(i) = size(cls_score_conv34, 1);
            %liu@0926 changed, see above
            output_h_conv34(i) = size(cls_score_conv34, 1);
        ...
        output_width_map_conv34 = containers.Map(input, output_w_conv34);
        output_height_map_conv34 = containers.Map(input, output_h_conv34);
        ...
        %0925
        save(output_map_save_name, 'output_width_map_conv34', 'output_height_map_conv34',...
