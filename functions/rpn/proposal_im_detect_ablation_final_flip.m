function [pred_boxes_conv4, scores_conv4, pred_boxes_conv5, scores_conv5, pred_boxes_conv6, scores_conv6] = proposal_im_detect_ablation_final_flip(conf, caffe_net, im, im_idx)
...
% --------------------------------------------------------    
    ...
    box_deltas_conv4 = output_blobs{2};
    ...
    [anchors_conv4, anchors_conv5, anchors_conv6] = proposal_locate_anchors_ablation_final(conf, size(im), conf.test_scales);
    ...
    scores_conv4 = output_blobs{5}(:, :, end);
    scores_conv4 = reshape(scores_conv4, size(output_blobs{2}, 1), size(output_blobs{2}, 2), []);
    % permute from [width, height, channel] to [channel, height, width], where channel is the
        % fastest dimension
    ...
    [pred_boxes_conv4, scores_conv4] = filter_boxes(5, pred_boxes_conv4, scores_conv4);
    ...
    % ===================================================== for conv5
    % Apply bounding-box regression deltas
    box_deltas_conv5 = output_blobs{3};
    ...
    assert(conf.test_binary == false);
    % use softmax estimated probabilities
    scores_conv5 = output_blobs{6}(:, :, end);
    scores_conv5 = reshape(scores_conv5, size(output_blobs{3}, 1), size(output_blobs{3}, 2), []);
    % permute from [width, height, channel] to [channel, height, width], where channel is the
        % fastest dimension
    ...
    [pred_boxes_conv5, scores_conv5] = filter_boxes(10, pred_boxes_conv5, scores_conv5);
    ...
    % ===================================================== for conv6
    % Apply bounding-box regression deltas
    box_deltas_conv6 = output_blobs{1};
    %featuremap_size = [size(box_deltas_conv5, 2), size(box_deltas_conv5, 1)];
    % permute from [width, height, channel] to [channel, height, width], where channel is the
        % fastest dimension
    ...
    assert(conf.test_binary == false);
    % use softmax estimated probabilities
    scores_conv6 = output_blobs{4}(:, :, end);
    scores_conv6 = reshape(scores_conv6, size(output_blobs{1}, 1), size(output_blobs{1}, 2), []);
    % permute from [width, height, channel] to [channel, height, width], where channel is the
        % fastest dimension
    ...
    % thresh as 20, may change later (conv5 minimal anchor size is 32)
    %0301 64-->50
    [pred_boxes_conv6, scores_conv6] = filter_boxes(50, pred_boxes_conv6, scores_conv6);
