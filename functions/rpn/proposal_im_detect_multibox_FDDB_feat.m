function [pred_boxes_conv4, scores_conv4, pred_boxes_conv5, scores_conv5, pred_boxes_conv6, scores_conv6, feat] = proposal_im_detect_multibox_FDDB_feat(conf, caffe_net, im, im_idx)
...
% --------------------------------------------------------    
    ...
    %0121 added: get feature map
    feat_conv4 = max(scores_conv4, [], 1);
    feat_conv4 = squeeze(feat_conv4);
    %figure,imshow(feat_conv4)
    ...
    %0121 added: get feature map
    feat_conv5 = max(scores_conv5, [], 1);
    feat_conv5 = squeeze(feat_conv5);
    feat_conv5 = imresize(feat_conv5, [size(feat_conv4,1) size(feat_conv4,2)]);
    %figure,imshow(feat_conv5);
    ...
    %0121 added: get feature map
    feat_conv6 = max(scores_conv6, [], 1);
    feat_conv6 = squeeze(feat_conv6);
    feat_conv6 = imresize(feat_conv6, [size(feat_conv4,1) size(feat_conv4,2)]);
    %figure,imshow(feat_conv6);
    %++++++++++++++++ average or max ++++++++++++++++++++++++
    feat_all = cat(3, feat_conv4, feat_conv5, feat_conv6);
    feat_max = max(feat_all, [], 3);
    feat_max_show = imresize(feat_max, [size(im,1) size(im,2)]);
    figure(1),imshow(feat_max_show);
    figure(2),imshow(im/255); % for comparison
    %feat_avg = sum(feat_all, 3)/3;
    %figure,imshow(feat_avg);
    % ++++++++++++++++ assign output ++++++++++++++++++++++
    feat = feat_max_show;
