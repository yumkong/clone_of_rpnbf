function [image_roidb, bbox_means_res23, bbox_stds_res23, bbox_means_res45, bbox_stds_res45]...
    = proposal_prepare_image_roidb_twopath_happy(conf, imdbs, roidbs, bbox_means_res23, bbox_stds_res23, bbox_means_res45, bbox_stds_res45)
...
% liu@1209: NOTE that XXX_conv4 is actually conv4, but for labor convenience, do not change  
    ...
    [image_roidb, bbox_means_res23, bbox_stds_res23, bbox_means_res45, bbox_stds_res45] = append_bbox_regression_targets(conf, ...
                            image_roidb, bbox_means_res23, bbox_stds_res23, bbox_means_res45, bbox_stds_res45);
end

function [image_roidb, bbox_means_res23, bbox_stds_res23, bbox_means_res45, bbox_stds_res45] = append_bbox_regression_targets(conf, image_roidb, ...
                            bbox_means_res23, bbox_stds_res23, bbox_means_res45, bbox_stds_res45)
    % means and stds -- (k+1) * 4, include background class
    
    num_images = length(image_roidb);
    % Infer number of classes from the number of columns in gt_overlaps
    image_roidb_cell = num2cell(image_roidb, 2);
    bbox_targets_res23 = cell(num_images, 1);
    bbox_targets_res45 = cell(num_images, 1);
    ...
    for i = 1:num_images
       ...
       [anchors_res23,anchors_res45, im_scales] = proposal_locate_anchors_twopath_happy(conf, image_roidb_cell{i}.im_size);
        
       gt_rois = image_roidb_cell{i}.boxes;
       gt_labels = image_roidb_cell{i}.class;
       im_size = image_roidb_cell{i}.im_size;
       bbox_targets_res23{i} = cellfun(@(x, y) ...
           compute_targets(conf, scale_rois(gt_rois, im_size, y), gt_labels,  x, image_roidb_cell{i}, y), ...
           anchors_res23, im_scales, 'UniformOutput', false);
       bbox_targets_res45{i} = cellfun(@(x, y) ...
           compute_targets(conf, scale_rois(gt_rois, im_size, y), gt_labels,  x, image_roidb_cell{i}, y), ...
           anchors_res45, im_scales, 'UniformOutput', false);
    end
    clear image_roidb_cell;
    for i = 1:num_images
        image_roidb(i).bbox_targets_res23 = bbox_targets_res23{i};
        image_roidb(i).bbox_targets_res45 = bbox_targets_res45{i};
    end
