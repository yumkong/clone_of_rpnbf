function save_model_path = proposal_train_widerface_conv3_4_ohem(conf, imdb_train, roidb_train, varargin)
...
% inputs
    ...
    %0115 added ohem_option: 0 (default):no ohem; 1: ohem
    ip.addParamValue('ohem_option',        false,          @isscalar); 
...
%% -------------------- Training -------------------- 
    ...
    while (iter_ <= max_iter)
        ...
        [net_inputs, scale_inds] = proposal_generate_minibatch_fun(conf, image_roidb_train(sub_db_inds), opts.ohem_option);
...
function val_results = do_validation(conf, caffe_solver, proposal_generate_minibatch_fun, image_roidb_val, shuffled_inds_val)
    ...
    for i = 1:length(shuffled_inds_val)
        ...
        % 0115: for validation, always use no_ohem (false) option
        [net_inputs, ~] = proposal_generate_minibatch_fun(conf, image_roidb_val(sub_db_inds), false);
