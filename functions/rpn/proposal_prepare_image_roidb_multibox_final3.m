function [image_roidb, bbox_means_conv4, bbox_stds_conv4, bbox_means_conv5, bbox_stds_conv5, bbox_means_conv6, bbox_stds_conv6]...
    = proposal_prepare_image_roidb_multibox_final3(conf, imdbs, roidbs, bbox_means_conv4, bbox_stds_conv4, bbox_means_conv5, bbox_stds_conv5, bbox_means_conv6, bbox_stds_conv6)
...
% --------------------------------------------------------   
    ...
    [image_roidb, bbox_means_conv4, bbox_stds_conv4, bbox_means_conv5, bbox_stds_conv5, bbox_means_conv6, bbox_stds_conv6] = append_bbox_regression_targets(conf, ...
                            image_roidb, bbox_means_conv4, bbox_stds_conv4, bbox_means_conv5, bbox_stds_conv5, bbox_means_conv6, bbox_stds_conv6);
end

function [image_roidb, means_conv4, stds_conv4, means_conv5, stds_conv5, means_conv6, stds_conv6] = append_bbox_regression_targets(conf, image_roidb, ...
                            means_conv4, stds_conv4, means_conv5, stds_conv5, means_conv6, stds_conv6)
    ...
    bbox_targets_conv6 = cell(num_images, 1);
    %parfor i = 1:num_images
    for i = 1:num_images
        % for fcn, anchors are concated as [channel, height, width], where channel is the fastest dimension.
       [anchors_conv4,anchors_conv5, anchors_conv6, im_scales] = proposal_locate_anchors_multibox_final3(conf, image_roidb_cell{i}.im_size);
       ...
       bbox_targets_conv6{i} = cellfun(@(x, y) ...
           compute_targets(conf, scale_rois(gt_rois, im_size, y), gt_labels,  x, image_roidb_cell{i}, y), ...
           anchors_conv6, im_scales, 'UniformOutput', false);
    end
    ...
    for i = 1:num_images
        ...
        %1122 added
        image_roidb(i).bbox_targets_conv6 = bbox_targets_conv6{i};
    end
    clear bbox_targets_conv4 bbox_targets_conv5 bbox_targets_conv6;
    ...
    if ~(exist('means_conv6', 'var') && ~isempty(means_conv6) && exist('stds_conv6', 'var') && ~isempty(stds_conv6))
        % Compute values needed for means and stds
        % var(x) = E(x^2) - E(x)^2
        class_counts = zeros(1, 1) + eps;
        sums = zeros(1, 4);
        ...
        means_conv6 = bsxfun(@rdivide, sums, class_counts);
        stds_conv6 = (bsxfun(@minus, bsxfun(@rdivide, squared_sums, class_counts), means_conv6.^2)).^0.5;
    end
    ...
    for i = 1:num_images
        for j = 1:length(conf.scales)
            ...
            gt_inds = find(targets(:, 1) > 0);
            if ~isempty(gt_inds)
                image_roidb(i).bbox_targets_conv6{j}(gt_inds, 2:end) = ...
                    bsxfun(@minus, image_roidb(i).bbox_targets_conv6{j}(gt_inds, 2:end), means_conv6);
                image_roidb(i).bbox_targets_conv6{j}(gt_inds, 2:end) = ...
                    bsxfun(@rdivide, image_roidb(i).bbox_targets_conv6{j}(gt_inds, 2:end), stds_conv6);
            end
        end
    end
end
