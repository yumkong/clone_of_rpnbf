function [aboxes_conv4, aboxes_conv5, aboxes_conv6] = proposal_test_MPRPN_singleRatio(conf, imdb, varargin)
...
%% testing
        ...
        for i = 1:num_images
            ...
            [boxes_conv4, scores_conv4, boxes_conv5, scores_conv5, boxes_conv6, scores_conv6] = proposal_im_detect_multibox_MPRPN_singleRatio(conf, caffe_net, im);
            %[boxes, scores] = proposal_im_detect_multibox(conf, caffe_net, im);
            
            fprintf(' time: %.3fs\n', toc(th)); 
            %1230 added
            scores_conv4 = scores_conv4(scores_conv4 >= 0.1,:);  %0101:0.1-->0.55
            boxes_conv4 = boxes_conv4(scores_conv4 >= 0.1,:);
            scores_conv5 = scores_conv5(scores_conv5 >= 0.1,:);
            boxes_conv5 = boxes_conv5(scores_conv5 >= 0.1,:);
            scores_conv6 = scores_conv6(scores_conv6 >= 0.1,:);
            boxes_conv6 = boxes_conv6(scores_conv6 >= 0.1,:);
