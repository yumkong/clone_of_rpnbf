function [input_blobs, random_scale_inds] = proposal_generate_minibatch_ablation_divanchor_noohem(conf, image_roidb)
...
%     end
    ...
    assert(mod(conf.batch_size_s4, num_images) == 0, ...
        sprintf('s4: num_images %d must divide BATCH_SIZE %d', num_images, conf.batch_size_s4));
    assert(mod(conf.batch_size_s8, num_images) == 0, ...
        sprintf('s8: num_images %d must divide BATCH_SIZE %d', num_images, conf.batch_size_s8));
    assert(mod(conf.batch_size_s16, num_images) == 0, ...
        sprintf('s16: num_images %d must divide BATCH_SIZE %d', num_images, conf.batch_size_s16));
    ...
    for i = 1:num_images
        [labels_conv4, label_weights_conv4, bbox_targets_conv4, bbox_loss_conv4] = ...
            sample_rois(conf, image_roidb(i).bbox_targets_s4, fg_rois_per_image_s4, rois_per_image_s4, im_scales(i), random_scale_inds(i));
        [labels_conv5, label_weights_conv5, bbox_targets_conv5, bbox_loss_conv5] = ...
            sample_rois(conf, image_roidb(i).bbox_targets_s8, fg_rois_per_image_s8, rois_per_image_s8, im_scales(i), random_scale_inds(i));
        [labels_conv6, label_weights_conv6, bbox_targets_conv6, bbox_loss_conv6] = ...
            sample_rois(conf, image_roidb(i).bbox_targets_s16, fg_rois_per_image_s16, rois_per_image_s16, im_scales(i), random_scale_inds(i));
