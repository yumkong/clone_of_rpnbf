function [anchors_res23, anchors_res45] = proposal_generate_anchors_twopath_flip(cache_name, varargin)
...
%         anchor_cache_file           = fullfile(anchor_cache_dir, 'anchors');
    ...
    try
        ...
        anchors_res23 = anchors([1:3 half_anchor_num+1:half_anchor_num+3], :);  % [8  16 32]
        anchors_res45 = anchors([4:7 half_anchor_num+4:half_anchor_num+7], :);  % [64 128 256 480]
        if ~opts.ignore_cache
            save(anchor_cache_file, 'anchors_res23', 'anchors_res45');
        end
    end
