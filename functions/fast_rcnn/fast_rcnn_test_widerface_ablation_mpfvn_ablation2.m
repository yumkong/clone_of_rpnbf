function mAP = fast_rcnn_test_widerface_ablation_mpfvn_ablation2(conf, imdb, roidb, varargin)
...
%%      testing 
        ...
        for i = 1:num_images
            ...
            if ~isempty(rpn_boxes)
                ...
                aboxes_2old_1new_p8{i} = [rpn_boxes (2*rpn_score+fastrcnn_score_p8)/9];
                aboxes_2old_1new_p7{i} = [rpn_boxes (2*rpn_score+fastrcnn_score_p7)/8];
                aboxes_2old_1new_p6{i} = [rpn_boxes (2*rpn_score+fastrcnn_score_p6)/7];
                aboxes_2old_1new_p5{i} = [rpn_boxes (2*rpn_score+fastrcnn_score_p5)/6];
                aboxes_2old_1new_p4{i} = [rpn_boxes (2*rpn_score+fastrcnn_score_p4)/5];
                aboxes_2old_1new_p3{i} = [rpn_boxes (3*rpn_score+fastrcnn_score_p3)/4];
                aboxes_2old_1new_p2{i} = [rpn_boxes (2*rpn_score+fastrcnn_score_p2)/3];
                aboxes_2old_1new_p1{i} = [rpn_boxes (rpn_score+fastrcnn_score_p1)/2];
            else
                aboxes_old{i} = [];
                aboxes_new{i} = [];
