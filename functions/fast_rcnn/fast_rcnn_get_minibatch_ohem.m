function [im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_blob] = fast_rcnn_get_minibatch_ohem(conf, image_roidb)
...
%     overlaps = image_roidb(1).max_overlaps;
    ...
    fg_inds = find(overlaps >= conf.fg_thresh); %1207 changed: keep all fgs that meet the threhold requirement
...
%     end
    ...
    %1207 changed: keep all fgs that meet the threhold requirement
