function save_model_path = fast_rcnn_train_widerface_ablation_ohem(conf, imdb_train, roidb_train, varargin)
...
%% training
    ...
    while (iter_ <= max_iter)
        ...
        [im_blob, rois_blob, labels_blob] = ...
            fast_rcnn_get_minibatch_onlycls_ohem(conf, image_roidb_train(sub_db_inds));
        ...
        if ~mod(iter_, opts.val_interval) 
            if opts.do_val
                ...
                for i = 1:length(shuffled_inds_val)
                    ...
                    % 0324: val still use orignal bg/fg methods
                    [im_blob, rois_blob, labels_blob] = ...
                        fast_rcnn_get_minibatch_onlycls(conf, image_roidb_val(sub_db_inds));
