function scores = fast_rcnn_im_detect_widerface_total_conv2345_MALF(conf, caffe_net, im, boxes, max_rois_num_in_gpu)
...
function [blob, im_scales] = get_image_blob(conf, im)
    ...
    %0410 changed to deal with limited gpu memory
...
function [rois_blob] = get_rois_blob(conf, im_rois, im_scale_factors, im_siz)
    [feat_rois, levels] = map_im_rois_to_feat_rois(conf, im_rois, im_scale_factors, im_siz);
    rois_blob = single([levels, feat_rois]);
end

function [feat_rois, levels] = map_im_rois_to_feat_rois(conf, im_rois, scales, im_siz)
