function [im_blob, rois_blob_s4, rois_cxt_blob_s4,rois_blob_s8, rois_cxt_blob_s8,rois_blob_s16, rois_cxt_blob_s16, labels_s4, labels_s8, labels_s16] = fast_rcnn_get_minibatch_mpfvn_cxt_realmp_ohem(conf, image_roidb)
...
%     overlaps = image_roidb(1).max_overlaps;
    ...
    % only randomly keep 30 fg candidates
    if length(fg_ind_s16) > 30
        fg_ind_s16 = fg_ind_s16(randperm(length(fg_ind_s16), 30));
    end
    
    % Select background ROIs as those within [BG_THRESH_LO, BG_THRESH_HI)
    bg_ind_s16 = find(overlap_s16 < conf.bg_thresh_hi & overlap_s16 >= conf.bg_thresh_lo);
    % only randomly keep 100 bg candidates
    if length(bg_ind_s16) > 100
        bg_ind_s16 = bg_ind_s16(randperm(length(bg_ind_s16), 100));
    end
