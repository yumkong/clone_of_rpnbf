function conf = fast_rcnn_config_widerface_mpfvn(varargin)
...
%
    ...
    ip.addParamValue('scales',          512,            @ismatrix); %600--900--512
    % Max pixel size of a scaled input image
    ip.addParamValue('max_size',        512,            @isscalar); %1000--1500--512
    % Images per batch
    ip.addParamValue('ims_per_batch',   4,              @isscalar); %2-->1
    % Minibatch size
    %ip.addParamValue('batch_size',      64,            @isscalar); %128-->160-->80(for 2 images)->64(2 images)
    ip.addParamValue('batch_size_s4',    64,            @isscalar); %0325: 32 (for 2 images)
    ip.addParamValue('batch_size_s8',    96,            @isscalar); %0325: 64 (for 3 images)
    ip.addParamValue('batch_size_s16',   32,            @isscalar); %0312: 48 (for 6 images) -> 24 (for 3 images)
    ...
    %% testing
    ip.addParamValue('test_scales',     512,            @isscalar); %900--512
    ip.addParamValue('test_max_size',   512,            @isscalar); %1500--512
