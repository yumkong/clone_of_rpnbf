function [pred_boxes] = fast_rcnn_bbox_transform_inv_yolo(boxes, box_deltas)
...
% --------------------------------------------------------
    ...
    dst_ctr_x = double(box_deltas(:, 1:4:end).^3);
    dst_ctr_y = double(box_deltas(:, 2:4:end).^3);
    dst_scl_x = double(box_deltas(:, 3:4:end).^3);
    dst_scl_y = double(box_deltas(:, 4:4:end).^3);
