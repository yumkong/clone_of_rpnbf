function save_model_path = fast_rcnn_train_widerface_conv3_4_batch2_feat(conf, imdb_train, roidb_train, varargin)
...
%% training
    ...
    while (iter_ <= max_iter)
        ...
        [im_blob, score_feat_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_weights_blob] = ...
            fast_rcnn_get_minibatch_batch2_feat(conf, image_roidb_train(sub_db_inds), score_maps_train(sub_db_inds));

        net_inputs = {im_blob, score_feat_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_weights_blob};
        ...
        if ~mod(iter_, opts.val_interval) 
            if opts.do_val
                ...
                for i = 1:length(shuffled_inds_val)
                    ...
                    [im_blob, score_feat_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_weights_blob] = ...
                        fast_rcnn_get_minibatch_batch2_feat(conf, image_roidb_val(sub_db_inds), score_maps_val(sub_db_inds));
...
%%  try to train/val with images which have maximum size potentially, to validate whether the gpu memory is enough  
    ...
    %0125 added
    score_feat_blob = single(zeros(size(im_blob,1)/4, size(im_blob,2)/4 ,7, conf.ims_per_batch));
