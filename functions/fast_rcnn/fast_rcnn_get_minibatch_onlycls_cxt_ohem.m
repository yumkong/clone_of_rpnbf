function [im_blob, rois_blob,rois_cxt_blob, labels_blob] = fast_rcnn_get_minibatch_onlycls_cxt_ohem(conf, image_roidb)
...
% --------------------------------------------------------
    ...
    %0322 added
    rois_cxt_blob = zeros(0, 5, 'single');
    ...
    for i = 1:num_images
        ...
        [feat_rois, feat_cxt_rois] = fast_rcnn_map_im_rois_to_feat_rois_cxt(conf, im_rois, im_scales(i), image_roidb(i).im_size);
        ...
        rois_cxt_blob_this_image = [batch_ind, feat_cxt_rois];
        rois_cxt_blob = [rois_cxt_blob; rois_cxt_blob_this_image];
    ...
    rois_cxt_blob = single(permute(rois_cxt_blob, [3, 4, 2, 1]));
...
%     overlaps = image_roidb(1).max_overlaps;
    ...
    %0324 added to avoid too much fg rois
    if length(fg_inds) > 200
        fg_inds = fg_inds(randperm(length(fg_inds), 200));
    end
...
%     end
    ...
    %0324 added to avoid too much bg rois
    if length(bg_inds) > 300
        bg_inds = bg_inds(randperm(length(bg_inds), 300));
    end
