function [im_blob, rois_blob_s4, rois_cxt_blob_s4,rois_blob_s8, rois_cxt_blob_s8,rois_blob_s16, rois_cxt_blob_s16, labels_blob] = fast_rcnn_get_minibatch_mpfvn_ohem(conf, image_roidb)
...
% --------------------------------------------------------
    ...
    for i = 1:num_images
        [labels, ~, im_rois_s4, im_rois_s8, im_rois_s16] = sample_rois(conf, image_roidb(i), fg_rois_per_image_s4, rois_per_image_s4, ...
...
function [labels, overlaps, rois_s4, rois_s8, rois_s16] = sample_rois(conf, image_roidb, fg_rois_per_image_s4, rois_per_image_s4, ...
                                            fg_rois_per_image_s8, rois_per_image_s8, fg_rois_per_image_s16, rois_per_image_s16)
...
%     overlaps = image_roidb(1).max_overlaps;
    ...
    % only randomly keep 50 fg candidates
    if length(fg_ind_s4) > 50
        fg_ind_s4 = fg_ind_s4(randperm(length(fg_ind_s4), 50));
    end
    
    % Select background ROIs as those within [BG_THRESH_LO, BG_THRESH_HI)
    bg_ind_s4 = find(overlap_s4 < conf.bg_thresh_hi & overlap_s4 >= conf.bg_thresh_lo);
    % only randomly keep 200 bg candidates
    if length(bg_ind_s4) > 200
        bg_ind_s4 = bg_ind_s4(randperm(length(bg_ind_s4), 200));
    end
    ...
    % only randomly keep 50 fg candidates
    if length(fg_ind_s8) > 50
        fg_ind_s8 = fg_ind_s8(randperm(length(fg_ind_s8), 50));
    end
    
    % Select background ROIs as those within [BG_THRESH_LO, BG_THRESH_HI)
    bg_ind_s8 = find(overlap_s8 < conf.bg_thresh_hi & overlap_s8 >= conf.bg_thresh_lo);
    % only randomly keep 200 bg candidates
    if length(bg_ind_s8) > 200
        bg_ind_s8 = bg_ind_s8(randperm(length(bg_ind_s8), 200));
    end
    ...
    % only randomly keep 50 fg candidates
    if length(fg_ind_s16) > 50
        fg_ind_s16 = fg_ind_s16(randperm(length(fg_ind_s16), 50));
    end
    
    % Select background ROIs as those within [BG_THRESH_LO, BG_THRESH_HI)
    bg_ind_s16 = find(overlap_s16 < conf.bg_thresh_hi & overlap_s16 >= conf.bg_thresh_lo);
    % only randomly keep 200 bg candidates
    if length(bg_ind_s16) > 200
        bg_ind_s16 = bg_ind_s16(randperm(length(bg_ind_s16), 200));
    end
