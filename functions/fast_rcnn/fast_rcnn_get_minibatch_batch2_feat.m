function [im_blob, score_feat_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_blob] = fast_rcnn_get_minibatch_batch2_feat(conf, image_roidb, score_maps)
...
% --------------------------------------------------------
    ...
    for i = 1:num_images
        ...
        %0125 added
        score_feat_blob = cat(4, score_feat_blob, score_maps{i});
        % Add to ROIs blob
    ...
    %0126 added
    score_feat_blob = permute(score_feat_blob, [2, 1, 3, 4]);
    %0126:scale it by a factor 10
    score_feat_blob = max(score_feat_blob, 0); % pseudo-relu
    score_feat_blob = score_feat_blob * 10; 
    ...
    %0126 added
    assert(~isempty(score_feat_blob));
