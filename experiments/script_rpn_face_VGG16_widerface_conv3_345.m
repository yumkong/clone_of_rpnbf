function script_rpn_face_VGG16_widerface_conv3_345()
...
opts.do_val                 = true; 
% model
model                       = Model.VGG16_for_rpn_widerface_conv3_345_ori(exp_name);
% cache base
...
model_name_base = 'vgg16_conv3_345';  % ZF, vgg16_conv5
%1009 change exp here for output
exp_name = 'VGG16_widerface_conv3_345';
...
output_map_name = 'output_map_conv345';  % output_map_conv3, output_map_conv5
output_map_save_name = fullfile(cache_data_this_model_dir, output_map_name);
[conf_proposal.output_width_map, conf_proposal.output_height_map] = proposal_calc_output_size_conv345(conf_proposal, ...
...
%%  train
fprintf('\n***************\nstage one RPN \n***************\n');
model.stage1_rpn            = Faster_RCNN_Train.do_proposal_train_widerface_conv345(conf_proposal, dataset, model.stage1_rpn, opts.do_val);
...
event_num = -1;
dataset                     = Dataset.widerface_all(dataset, 'test', false, event_num, cache_data_this_model_dir, model_name_base);
Faster_RCNN_Train.do_proposal_test_widerface_conv345(conf_proposal, model.stage1_rpn, dataset.imdb_test, dataset.roidb_test, cache_name, method_name, nms_option_test);
