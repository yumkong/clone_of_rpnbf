function script_rpn_face_VGG16_widerface_conv4_compare()
...
if ispc
    opts.caffe_version          = 'caffe_faster_rcnn_win_cudnn_final'; %caffe_faster_rcnn_win
    cd('D:\\RPN_BF_master');
elseif isunix
    opts.caffe_version          = 'caffe_faster_rcnn_rfcn_ohem_final_noprint';
...
opts.do_val                 = true; 
% model
model                       = Model.VGG16_for_rpn_widerface_conv4_compare(exp_name);
% cache base
...
exp_name = 'VGG16_widerface_conv4_compare';
...
%%  train
fprintf('\n***************\nstage one RPN \n***************\n');
model.stage1_rpn            = Faster_RCNN_Train.do_proposal_train_widerface_CMS(conf_proposal, dataset, model.stage1_rpn, opts.do_val);
...
method_name = 'RPN-ped';
nms_option_test = 3;
% 1229 added, use all validation set to evaluate
dataset                     = Dataset.widerface_all(dataset, 'test', false, event_num_test, cache_data_this_model_dir, model_name_base);
Faster_RCNN_Train.do_proposal_test_widerface_compare(conf_proposal, model.stage1_rpn, dataset.imdb_test, dataset.roidb_test, cache_name, method_name, nms_option_test);
