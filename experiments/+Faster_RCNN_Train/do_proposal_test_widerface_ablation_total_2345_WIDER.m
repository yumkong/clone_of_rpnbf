function do_proposal_test_widerface_ablation_total_2345_WIDER(conf,conf_fast_rcnn, model_stage,model_stage_fast, imdb, roidb, nms_option)
...
%%      testing 
    ...
    try
        ...
        for i = 1:length(aboxes_s4)
            ...
            if 1      
                ...
                if ~isempty(bbs_gt)
                    if ~isempty(bbs_all)
                        overlaps = boxoverlap(bbs_all(:,1:4), bbs_gt);  
                        max_per_predict_ols = max(overlaps, [], 2);
                        %hard fp index
                        idx_tmp = (bbs_all(:,5) >= 1.098) & (max_per_predict_ols <= 0.3);
                    end
                else
                    if ~isempty(bbs_all)
                        idx_tmp = (bbs_all(:,5) >= 1.098);
                    end
                end
                if ~isempty(idx_tmp)
                    hard_fp_mat = cat(1, hard_fp_mat, [i*ones(sum(idx_tmp), 1) bbs_all(idx_tmp,:)]);
                end
            end
    ...
    %             bbs_gt(invalid_idx, :) = [];
                ...
                if ~isempty(bbs_all)
                    ...
                    bbs_all = bbs_all(bbs_all(:,5)>=0.995,:);%1.097
                    %bbs_all(:,5) = bbs_all(:,5) - 0.1;
                end
                if ~isempty(bbs_all)
        ...
        [~, scores_ind] = sort(hard_fp_mat(:,6), 'descend');
        hard_fp_mat = hard_fp_mat(scores_ind, :);
