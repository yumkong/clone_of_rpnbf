function do_proposal_test_widerface_multibox_ohem_right(conf, model_stage, imdb, roidb, cache_name, method_name, nms_option)
...
function aboxes = boxes_filter(aboxes, per_nms_topN, nms_overlap_thres, after_nms_topN, use_gpu)
    ...
    if per_nms_topN > 0
        ...
        %1202 added: if no pre selection by per_nms_topN, only keep the boxes with a score >= 0.5 (intuitive)
        % aboxes_conv4{i} = aboxes_conv4{i}(aboxes_conv4{i}(:, end) > score_thresh_conv4, :);
        aboxes = cellfun(@(x) x(x(:, end) > 0.5, :), aboxes, 'UniformOutput', false);
    end
    % do nms
