function do_proposal_test_widerface_twopath_happy_batch2_e1_e11(conf, model_stage, imdb, roidb, nms_option)
...
%     fprintf('score_threshold res23 = %f, res45 = %f\n', score_thresh_res23, score_thresh_res45);
    ...
    for i = 1:length(aboxes_res23)
        % 0203: no more thresholding, use all boxes to compute recall rate
        %aboxes_res23{i} = aboxes_res23{i}(aboxes_res23{i}(:, end) > 0.65, :);  %0.99
        %aboxes_res45{i} = aboxes_res45{i}(aboxes_res45{i}(:, end) > 0.65, :);%0.99
        ...
        %0203: sort in descending order
        if ~isempty(aboxes_nms{i})
            [~, scores_ind] = sort(aboxes_nms{i}(:, 5), 'descend');
            aboxes_nms{i} = aboxes_nms{i}(scores_ind, :);
        end
        ...
        if ~isempty(gts)
            this_gt_num = size(gts, 1);
            gt_num = gt_num + this_gt_num;
            if ~isempty(aboxes_nms{i})
                this_pred_num = min(2*this_gt_num, size(aboxes_nms{i}, 1));
                %rois = aboxes_nms{i}(:, 1:4);
                rois = aboxes_nms{i}(1:this_pred_num, 1:4);
