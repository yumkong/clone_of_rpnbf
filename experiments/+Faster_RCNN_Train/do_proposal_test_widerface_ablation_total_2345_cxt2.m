function do_proposal_test_widerface_ablation_total_2345_cxt2(conf,conf_fast_rcnn, model_stage,model_stage_fast, imdb, roidb, nms_option)
...
%%      testing 
    ...
    for i = 1:length(aboxes_s4)
        ...
        bbs_all = aboxes_v1{i};
        %0420: make it less than 1
        bbs_all(:, 5) = bbs_all(:, 5) - 0.1;
        %0420 added for removing face in face
        bbs_all = pseudoNMS_v8_stage2(bbs_all);
        fprintf(fid1, '%d\n', size(bbs_all, 1));
        if ~isempty(bbs_all)
...
%             bbs_gt(invalid_idx, :) = [];
            ...
            imshow(img);  %im(img)
            hold on
            bbs_all_show = bbs_all(bbs_all(:,5)>=0.99,:);
            if ~isempty(bbs_all)
                  bbs_all_show(:, 3) = bbs_all_show(:, 3) - bbs_all_show(:, 1) + 1;
                  bbs_all_show(:, 4) = bbs_all_show(:, 4) - bbs_all_show(:, 2) + 1;
                  %bbs_all_show(:, 5) = bbs_all_show(:, 5) - 0.1;
                  bbApply('draw',bbs_all_show,'g');
            end
%             if ~isempty(bbs_gt)
