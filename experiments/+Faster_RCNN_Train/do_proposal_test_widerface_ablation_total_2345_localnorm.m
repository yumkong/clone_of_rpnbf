function do_proposal_test_widerface_ablation_total_2345_localnorm(conf,conf_fast_rcnn, model_stage,model_stage_fast, imdb, roidb, nms_option)
...
%%      testing 
    ...
    for i = 1:length(aboxes_s4)
        ...
        if ~isempty(aboxes_old{i})
            ...
            fastrcnn_score2 = (fastrcnn_score - min(fastrcnn_score))/(max(fastrcnn_score) - min(fastrcnn_score))*0.2 + 0.8;
        ...
        if ~isempty(rpn_boxes)
            ...
            aboxes_v1{i} = [rpn_boxes (rpn_score + 0.1 * fastrcnn_score2)];
            aboxes_v2{i} = [rpn_boxes (rpn_score + 0.3 * fastrcnn_score2)];
            aboxes_v3{i} = [rpn_boxes (rpn_score + 0.5 * fastrcnn_score2)];
            aboxes_v4{i} = [rpn_boxes (rpn_score + 0.7 * fastrcnn_score2)];
            aboxes_v5{i} = [rpn_boxes (rpn_score + 0.9 * fastrcnn_score2)];
        else
            aboxes_old{i} = [];
            aboxes_new{i} = [];
            aboxes_v1{i} = [];
