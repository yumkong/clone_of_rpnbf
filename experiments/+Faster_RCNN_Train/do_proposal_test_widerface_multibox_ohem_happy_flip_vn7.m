function do_proposal_test_widerface_multibox_ohem_happy_flip_vn7(conf, model_stage, imdb, roidb, cache_name, method_name, nms_option)
    ...
    for i = 1:length(aboxes_conv4)
        ...
        %0103 added: show the combined boxes
        if show_image
            img = imread(imdb.image_at(i));  
            %draw before NMS
            bbs = aboxes_nms{i};
