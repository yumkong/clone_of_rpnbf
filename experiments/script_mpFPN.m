function script_mpFPN()
% script_VGG16_widerface_multibox_ablation_noohem_nodivanchor()
% --------------------------------------------------------
% Yuguang Liu
...
run(fullfile(fileparts(fileparts(mfilename('fullpath'))), 'old_startup')); % startup
%% -------------------- CONFIG --------------------
%0930 change caffe folder according to platform
if ispc
    opts.caffe_version          = 'caffe_centerloss_exp'; %'caffe_faster_rcnn_win_cudnn_newL1'; 
    cd('D:\\RPN_BF_master');
elseif isunix
    ...
    opts.caffe_version          = 'caffe_centerloss_exp'; %'caffe_faster_rcnn_newL1'; 
...
opts.do_val                 = true; 
% model
model                       = Model.VGG16_for_multibox_batch2_ablation_nodivanchor(exp_name);
% cache base
...
exp_name = 'VGG16_widerface_multibox_ablation_noohem_nodivanchor';
...
% %% -------------------- TRAIN --------------------
% conf
conf_proposal          = proposal_config_widerface_ablation_nodivanchor('image_means', model.mean_image, 'feat_stride_s4', model.feat_stride_s4,...
...
%%  train
fprintf('\n***************\nstage one RPN \n***************\n');
model.stage1_rpn            = Faster_RCNN_Train.do_proposal_train_widerface_ablation_final_noohem(conf_proposal, dataset, model.stage1_rpn, opts.do_val);
