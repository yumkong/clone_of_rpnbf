function detector = DeepTrain_otf_trans_ratio_context( varargin )
...
%             showboxes2(im, rois(idx).boxes(sel_idx(nms_sel_idxes), :))
            ...
            if ~isempty(sel_boxes)
                ...
                %1215: rois_get_features_ratio --> rois_get_features_ratio_context
                fg_feat_cell{idx} = rois_get_features_ratio_context(opts.conf, opts.caffe_net, im, sel_boxes, opts.max_rois_num_in_gpu, opts.ratio);
...
%                sel_feat = sel_feat(retain_idx, :);
           ...
           %1027 changed: rois_get_features_ratio --> rois_get_features_ratio2
           %1215 rois_get_features_ratio --> rois_get_features_ratio_context
           sel_feat = rois_get_features_ratio_context(opts.conf, opts.caffe_net, im, sel_box, opts.max_rois_num_in_gpu, opts.ratio);
           %1206 added
