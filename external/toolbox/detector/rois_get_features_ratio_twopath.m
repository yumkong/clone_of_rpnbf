function [feats] = rois_get_features_ratio_twopath(conf, caffe_net, im, boxes, max_rois_num_in_gpu, ratio)
% 1210 created by Yuguang Liu
% extract features according to bbox sizes:
% if box_height < 14 or bbox_width < 14, use context-based features [l r t
% b] = [0.5 0.5 0.2 0.8]
% otherwise, box_height >= 14 and bbox_width >= 14, use original features
% --------------------------------------------------------
    %1210 added
    box_width_all = boxes(:,3)-boxes(:,1) + 1;
    box_height_all = boxes(:,4)-boxes(:,2) + 1;
    small_box_idx = (box_width_all < 14) & (box_height_all < 14);

    % liu@1001: ratio = 1, so no bbox width and height changes
    outer_box_ratio = ratio;
    %1004 changed, 1210 changed
    boxes_width_change_tmp = (boxes(:,3)-boxes(:,1) + 1)*(outer_box_ratio-1)/2;
    boxes_width_change = zeros(size(boxes_width_change_tmp));
    boxes_width_change(small_box_idx) = boxes_width_change_tmp(small_box_idx);
    ...
    boxes_top_change_tmp = (boxes(:,4)-boxes(:,2) + 1) * top_ratio;
    boxes_top_change = zeros(size(boxes_top_change_tmp));
    boxes_top_change(small_box_idx) = boxes_top_change_tmp(small_box_idx);
    
    boxes_bottom_change_tmp = (boxes(:,4)-boxes(:,2) + 1) * bottom_ratio;
    boxes_bottom_change = zeros(size(boxes_bottom_change_tmp));
    boxes_bottom_change(small_box_idx) = boxes_bottom_change_tmp(small_box_idx);
